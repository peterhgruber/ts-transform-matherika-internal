...
def main():
    """
    Forecast runner (TimesFM 2.0 / 2.5 only):
    - Reads runfiles/forecast_*.txt
    - Selects only TimesFM 2.0 and 2.5 jobs
    - Skips jobs whose forecast already exists
    - Dynamically imports forecast functions and saves results
    """
    ...
    for runfile_path in runfiles:
        ...
        if model_name.startswith("timesfm_model_2_5"):
    ...
    for model_name, jobs in grouped_runfiles.items():
        ...
        try:
            model_module = importlib.import_module(f"models.timesfm_models.{model_name}")
